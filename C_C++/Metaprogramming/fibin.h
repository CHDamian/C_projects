...
template <long long n, typename ValueType>
struct FibWrapper {
    static_assert (n >= 2);
    static constexpr ValueType value = FibWrapper<n - 1, ValueType>::value + FibWrapper<n - 2, ValueType>::value;
};
...
template <unsigned id, typename Value>
struct Binding {};
...
template <unsigned id, typename Body, typename ValueType, typename Environment>
...
struct If {
    template <typename VType, typename Env>
    using unrollType = details::GetTemplateUnrollType<
    details::GetUnrollType<details::IfHelper <
    details::GetTemplateUnrollType<Condition, VType, Env>::bvalue,
    Then,
    Else
    >>, VType, Env>;
};
...
struct Eq {
    ...
    details::GetTemplateUnrollType<Left, VType, Env>::value ==
    details::GetTemplateUnrollType<Right, VType, Env>::value
    >;
};

// Converting varaible name to id. If variables contains forbidden chars return 0.
template<size_t N>
constexpr unsigned Var(const char (&name) [N]) {
...
struct Ref {
    static_assert(id > 0);
    template <typename VType, typename Env>
    using unrollType = details::GetUnrollType<details::FindVar<id, Env>>;
};

// Computes expression with new variable.
template <unsigned id, typename Value, typename Expression>
struct Let {
    static_assert(id > 0);
    template <typename VType, typename Environment>
    using unrollType = details::GetTemplateUnrollType<
    Expression, VType, details::GetUnrollType<details::MergeEnv<details::Env<details::Binding<id,
    details::GetTemplateUnrollType<Value, VType, Environment>>>, Environment>>>;
};

// Fibin Lambda type.
template <unsigned id, typename Body>
struct Lambda {
    static_assert(id > 0);
    template <typename VType, typename Environment>
...
public:
    ...
    static constexpr eval(){
        return details::GetTemplateUnrollType<Expression, ValueType, details::EmptyEnv>::value;
    }

    // Is callable when ValueType is not integral. Prints information about unsupported type.
    template <typename Expression, typename T = ValueType>
